...
def reef_board() -> str:
    ...
    if archive.get("total_reefs", 0) > 0:
        past = f' · <a href="{SCRAPBOOK_URL}#reefs">see past reefs 📖</a>'
    return (
        '<h2 align="center">🪸 The Reef</h2>\n\n'
        f'<p align="center">{img}</p>\n\n'
        f'<p align="center">🪸 <i>This reef grows when I ship. Last tide: {last} · '
        f'this is reef #{reef_no}.</i>{past}</p>\n'
    )
...
def bottle_board() -> str:
    ...
    out = ['<h2 align="center">🍾 Message in a Bottle</h2>\n']
    out.append(
        '<div align="center">\n'
        '<table><tr>'
    ...
    if not bottles:
        out.append('\n<p align="center"><i>🌊 The sea is calm. '
                   'Be the first to toss a bottle.</i></p>\n')
    else:
        ...
        for i, b in enumerate(bottles):
            ...
            msg = b.get("msg", "")
            out.append(f'<p align="center">{g} <i>"{msg}"</i> — @{who}, {when}</p>\n')
    return "".join(out) + "\n"
...
def creature_board() -> str:
    ...
    return (
        ...
        f'<p align="center">{img}</p>\n\n'
        '<div align="center">\n\n'
        f"<b>This round we're picking the <code>{rnd}</code></b> "
        f"(round {round_idx} of {n_rounds}). Vote one feature per visit:\n\n"
        + " · ".join(links) + "\n\n"
        f"{sb}\n\n"
        "</div>\n"
    )
...
def build() -> str:
    ...
    footer = (
        "\n---\n\n"
        f'<p align="center"><sub>🌊 This profile is alive — the reef grows from my '
        f"commits, the bottle wall and creature are built by visitors like you. "
        f"Last assembled {stamp} UTC · generated by <code>build_readme.py</code>, "
        f"never hand-edited.</sub></p>\n"
    )
    return body + footer
